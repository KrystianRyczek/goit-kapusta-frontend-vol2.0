...
export const useTransactionTable = (activeSheet) => {
  ...
  const transactionData = newData.reduce((accumulator, currentValue) => {
    currentValue = {
      ...currentValue,
      date: parseFloat(
        currentValue.date.slice(0, 10).replace('-', '').replace('-', '')
      ),
    };
    return [...accumulator, currentValue];
  }, []);
  const transactionDataSort = transactionData.sort(
  ...
  const tableDataSort = transactionDataSort.reduce(
    (accumulator, currentValue) => {
      currentValue = {
        ...currentValue,
        date:
          `${currentValue.date.toString().slice(0, 4)}-` +
